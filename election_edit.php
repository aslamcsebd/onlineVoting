...
<?php  
   ...
   if (isset($_POST['edit_election'])) {
    ...
    $sql ="update $name set title='$title', date='$date', details='$details' where id='$row[id]' ";
      $result =  mysqli_query($vote,$sql);
      if ($result) {
         $_SESSION['election_edit_successfully']=true;
         echo "<script>window.location.href='election.php?type=$name&election=$name'</script>";
      }
   }  
?>
                  ...
                  <form action="" method="POST">
                     ...
                     <tr>
                        ...
                        <td class="text-left">
                           ...
                           <button type="submit" name="edit_election" class="btn btn-primary" onclick="return confirm('Are you sure?')">Edit Now
                           </button>
                        </td>                         
                     </tr>                     
                  </form>            
